import { OverrideToken, AliasToken } from 'antd/es/theme/interface'

export const globalTokens: Partial<AliasToken> = {
  fontFamily: `'Manrope', sans-serif`,
  lineWidthFocus: 0,
  controlOutline: 'var(--primary-2)',
  controlOutlineWidth: 1,
  borderRadius: 2,
  colorPrimaryBg: 'var(--primary-1)',
  colorPrimaryBgHover: 'var(--primary-2)',
  colorPrimaryBorder: 'var(--primary-2)',
  colorPrimaryBorderHover: 'var(--primary-3)',
  colorPrimaryHover: 'var(--primary-4)',
  colorPrimary: 'var(--primary-1)',
  colorPrimaryActive: 'var(--primary-1)',
  colorPrimaryTextHover: 'var(--primary-4)',
  colorPrimaryText: 'var(--primary-2)',
  colorPrimaryTextActive: 'var(--primary-2)'
}

export const componentTokens: OverrideToken = {
  Button: {
    colorPrimaryBg: 'var(--primary-1)',
    colorPrimaryBgHover: 'var(--primary-1)',
    colorPrimaryBorder: 'var(--primary-2)',
    colorPrimaryBorderHover: 'var(--primary-3)',
    colorPrimaryHover: 'var(--primary-4)',
    colorPrimary: 'var(--primary-1)',
    colorPrimaryActive: 'var(--primary-3)',
    colorPrimaryTextHover: 'var(--primary-4)',
    colorPrimaryText: 'var(--white)',
    colorPrimaryTextActive: 'var(--white)',
    borderRadius: 8,
    borderRadiusLG: 8,
    borderRadiusSM: 8,
    paddingInline:40,
    paddingBlock:16,
    fontSize:16, 
    contentLineHeight:1.2, 
    fontWeight:400,
    defaultShadow: 'none',
    primaryShadow: 'none'
  },
  Form:{
    labelColor:'var(--primary-4)',
    labelFontSize:14,
    labelHeight:32,
    labelRequiredMarkColor:'var(--error)'
  },
  Input:{
    paddingInline:12,
    paddingBlock:10,
    borderRadius: 8,
    borderRadiusLG: 8,
    borderRadiusSM: 8,
    fontSize:14, 
    colorBorder:'var(--primary-4)',
    colorText:"var(--primary-4)",
    colorTextPlaceholder:"var(--primary-4)",
   },
  Select: {
    colorBgElevated: 'var(--neutral-0)',
    selectorBg: 'var(--neutral-0)',
    colorText: 'var(--neutral-900)',
    colorPrimaryBg: 'var(--primary-0)',
    colorPrimaryBgHover: 'var(--primary-100)',
    colorPrimaryBorder: 'var(--primary-200)',
    colorPrimaryBorderHover: 'var(--primary-300)',
    colorPrimaryHover: 'var(--primary-400)',
    colorPrimary: 'var(--primary-500)',
    colorPrimaryActive: 'var(--primary-600)',
    colorPrimaryTextHover: 'var(--primary-400)',
    colorPrimaryText: 'var(--primary-500)',
    colorPrimaryTextActive: 'var(--primary-600)',
    optionSelectedBg: 'var(--primary-100)'
  }
}
